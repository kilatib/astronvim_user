...
local copyright_holder = "Open Assessment Technologies S.A."

local function get_header_lines(copyright_line, original_work_line)
	local spdx_copyright = copyright_line
		or string.format("// SPDX-FileCopyrightText: %s %s", current_year, copyright_holder)
	local original_work = original_work_line
		or string.format("// Copyright (C) %s (original work) %s", current_year, copyright_holder)
...
local function find_original_work_line(lines)
	for _, line in ipairs(lines) do
		if line:find("Copyright.-%(original work%)") then
			local normalized = line:gsub("^%s*%*?%s*", ""):gsub(";%s*$", "")
...
local function get_first_copyright_year(line)
	if not line then
		return nil
	end

	return line:match("Copyright.-(%d%d%d%d)")
end

local function get_spdx_copyright_line(lines)
	local copyright_line = find_copyright_line(lines)
	if copyright_line then
		return copyright_line
	end

	local first_year = get_first_copyright_year(find_original_work_line(lines))
	if first_year then
		local years = first_year == current_year and first_year or first_year .. "-" .. current_year
		return string.format("// SPDX-FileCopyrightText: %s %s", years, copyright_holder)
	end
end
...
function M.update_or_add_license_header()
	...
	-- Preserve existing SPDX copyright text. For legacy headers, use the first
	-- original-work year as the beginning of the new SPDX copyright period.
	local replacement = get_header_lines(get_spdx_copyright_line(all_lines), find_original_work_line(all_lines))
